...
internal static class RunTestsMethodGenerator
{
        ...
        IMethodSymbol[] testMethods)
    {
        ...
        sb.AppendLine("        var aggregateException = finishedTest.Exception as AggregateException;");
        sb.AppendLine("        var exceptions = aggregateException != null");
        sb.AppendLine("            ? (IList<Exception>)aggregateException.Flatten().InnerExceptions");
        sb.AppendLine("            : new[] {finishedTest.Exception};");
        sb.AppendLine("        foreach (var exception in exceptions)");
        sb.AppendLine("        {");
        sb.AppendLine(@"            Console.WriteLine(""x {0}: {1}"", finishedTest.Name, exception.Message);");
        sb.AppendLine("        }");
        ...
        sb.AppendLine(@"        Console.WriteLine();");
        sb.AppendLine("        var aggregateException = failedTest.Exception as AggregateException;");
        sb.AppendLine("        var exceptions = aggregateException != null");
        sb.AppendLine("            ? (IList<Exception>)aggregateException.Flatten().InnerExceptions");
        sb.AppendLine("            : new[] {failedTest.Exception};");
        sb.AppendLine("        foreach (var exception in exceptions)");
        sb.AppendLine("        {");
        sb.AppendLine("            Console.WriteLine(exception.Message);");
        sb.AppendLine(@"            Console.WriteLine();");
        sb.AppendLine(@"            Console.WriteLine(""Exception type: {0}"", exception.GetType());");
        sb.AppendLine(@"            Console.WriteLine(""Stack trace: {0}"", exception.StackTrace);");
        sb.AppendLine(@"            Console.WriteLine();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
